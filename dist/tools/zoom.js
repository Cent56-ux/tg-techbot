"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createMeeting = createMeeting;
const axios_1 = __importDefault(require("axios"));
const config_1 = require("../config");
let cached = null;
async function zoomToken() {
    const now = Math.floor(Date.now() / 1000);
    if (cached && cached.exp - now > 60)
        return cached.token;
    const url = `https://zoom.us/oauth/token?grant_type=account_credentials&account_id=${config_1.CFG.zoom.accountId}`;
    const r = await axios_1.default.post(url, {}, {
        auth: { username: config_1.CFG.zoom.clientId, password: config_1.CFG.zoom.clientSecret }
    });
    cached = { token: r.data.access_token, exp: now + r.data.expires_in };
    return cached.token;
}
async function createMeeting(input) {
    const token = await zoomToken();
    const body = {
        topic: input.title,
        type: 2,
        start_time: input.start_at,
        duration: input.duration_minutes,
        timezone: 'Europe/Berlin',
        settings: { join_before_host: false, approval_type: 2, waiting_room: false }
    };
    const resp = await axios_1.default.post('https://api.zoom.us/v2/users/me/meetings', body, {
        headers: { Authorization: `Bearer ${token}` }
    });
    return { join_url: String(resp.data.join_url), meeting_id: String(resp.data.id) };
}
